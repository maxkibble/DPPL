...
(*- Start deal with let structure with tuple -*)
  | TmPLet(fi, xs, t1, t2) when (isval ctx t1) ->
      (match (xs, t1) with
         ([x], TmRecord(fi_, [(l, v)])) -> TmLet(fi, x, v, t2)
      | (x::rest1, TmRecord(fi_, (l, v)::rest2)) -> TmLet(fi, x, v, TmPLet(fi, rest1, TmRecord(fi_, rest2), t2))
      | (_, _) -> raise NoRuleApplies)
  | TmPLet(fi, xs, t1, t2) ->
      let t1' = eval1 ctx t1 in TmPLet(fi, xs, t1', t2)
(*- End deal with let structure with tuple -*)
